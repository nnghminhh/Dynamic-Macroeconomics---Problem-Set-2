...
classdef model
    methods(Static)
        ...
        function par = setup()            
            ...
            par.nu = 0.5; % Frisch Elasticity: Higher values of this mean that them labor choice becomes more sensitive to productivity shocks.
            ...
            data_G = readtable("G_t_new.csv", 'VariableNamingRule', 'preserve'); % Import G_t from real data.

            par.Gmat = [data_G.("G_t")]; % Store G_t as a matrix.
            par.Gmat = par.Gmat/par.Gmat(1);
            rng(par.seed); % Ensure reproducibility
            par.Nmat = 0.1 + (0.8 - 0.1) * rand(size(par.Gmat));
